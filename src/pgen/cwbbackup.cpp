...
void OrbitCalc(Real t, Real ruser_phase);
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  OrbitCalc(pmy_mesh->time, 0.0);
  ruser_meshblock_data[0](0) = orbitPhase;
...
void MeshBlock::UserWorkInLoop() {
  ...
  OrbitCalc(pmy_mesh->time, ruser_meshblock_data[0](0)); 
  ruser_meshblock_data[0](0) = orbitPhase;
...
void OrbitCalc(Real t, Real ruser_phase){
  ...
  // **Test to see if restart has occurred**
  // Check to see if the simulation has just started and if there is a nonzero
  // value for the history file saved value for phase, if both are the case,
  // a restart has occurred, and simulation takes the saved value of phase
  // Phase is updated after each time it is called in
  // MeshBlock::ProblemGenerator() and MeshBlock::UserWorkInLoop()
  static bool firstorbit = true;
  if (firstorbit && ruser_phase != 0.0) {
    if (Globals::my_rank == 0) {
      printf("! Restart detected in OrbitCalc, using phase of %.3e\n",ruser_phase);
    }
    phase = ruser_phase;
  }
  firstorbit = false;
